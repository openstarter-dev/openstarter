...
describe("notes routes", () => {
  it("POST /api/notes creates a note under the envelope", async () => {
  ...
  it("GET /api/notes lists notes scoped to the requesting user", async () => {
    ...
    const alien = await request("/api/notes", {
  ...
  it("GET /api/notes/:id returns 404 envelope for missing note", async () => {
    const response = await request("/api/notes/note_nonexistent");
    expect(response.status).toBe(404);
    const body = await response.json();
    expect(body).toEqual({ code: -1, message: "note not found" });
  });

  it("POST /api/notes rejects invalid body with 422", async () => {
    const response = await request("/api/notes", {
