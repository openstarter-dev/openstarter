...
// 流程中调用 `getUrl(...)` 组装面向前端的验证 / 回调 URL；`index.ts` 亦 re-export 之。
//
// 语义（恢复 TurboStarter 既有行为，不裁剪）：将 better-auth 生成的回调 `url` 包裹到
// 应用自身的验证路由之下，使前端（web / 扩展 / 移动端）先接管再完成校验；未给定
// `url` / `type` 时返回应用基址（调用方可继续追加 query，如组织邀请的 invitationId）。
 ...
 * 组装验证 / 回调 URL。
 *
 * 给定 `url` 与 `type` 时，返回指向应用验证路由（`/auth/verify/{type}`）、并以
 * `callbackURL` 携带原始回调链接的 URL；否则返回应用基址。
 */
export const getUrl = ({ request, url, type }: GetUrlParams): URL => {
  const base = getBaseUrl(request);
  if (url && type) {
    const target = new URL(`/auth/verify/${type}`, base);
    target.searchParams.set("callbackURL", url);
    return target;
  }
  return new URL(base);
};
